...
module Views
  module Configurazione
    module ProgressivoNotaDiCreditoPanel
      include Views::Base::Panel
      include Helpers::MVCHelper
      
      def ui

        model :progressivo_nc => {:attrs => [:anno_rif, :progressivo],
                                               :alias => :progressivo
        }
        controller :configurazione

        logger.debug('initializing ProgressivoNotaDiCreditoPanel...')
        xrc = Xrc.instance()

        xrc.find('chce_anno_rif', self, :extends => ChoiceStringField)

        subscribe(:evt_progressivo_nc) do |data|
      ...
      def chce_anno_rif_select(evt)
        begin
          ...
          if self.progressivo = ctrl.search_progressivo(Models::ProgressivoNc, anno)
