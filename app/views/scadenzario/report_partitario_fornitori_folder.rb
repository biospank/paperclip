...
module Views
  module Scadenzario
    module ReportPartitarioFornitoriFolder
...
#      include Helpers::ODF::Report
      ...
      attr_accessor :totale_fatture, :totale_nc, :totale_pagamenti, :ripresa_saldo, :totale_saldo, :active_filter
      
      def ui
        model :filtro => {:attrs => []}
        controller :scadenzario

        logger.debug('initializing ReportPartitarioFornitoriFolder...')
        xrc = Xrc.instance()
        
        xrc.find('chce_anno', self, :extends => ChoiceStringField)

        subscribe(:evt_anni_contabili_fattura_fornitore_changed) do |data|
        ...
        xrc.find('lstrep_partitario', self, :extends => ReportField) do |list|
        ...
        xrc.find('cpt_totale_pagamenti', self)
      ...
      def reset_folder()
        lstrep_partitario.reset()
        result_set_lstrep_partitario.clear()
        reset_totali()
      end
      ...
      def btn_ricerca_click(evt)
        ...
        begin
          Wx::BusyCursor.busy() do
            ...
            self.result_set_lstrep_partitario = ctrl.report_partitario_fornitori()
            lstrep_partitario.display_matrix(result_set_lstrep_partitario)
            ...
            if optional_conditions_enabled?
              ...
              self.lbl_totale_saldo.label = Helpers::ApplicationHelper.currency((totale_fatture - totale_nc - totale_pagamenti) + ripresa_saldo) 
            end
      ...
      def render_header(opts={})
        ...
        header.write(
          ERB.new(
            IO.read(Helpers::ScadenzarioHelper::PartitarioHeaderTemplatePath)
          ).result(binding)
        )
      ...
      def render_body(opts={})
        body.write(
          ERB.new(
            IO.read(Helpers::ScadenzarioHelper::PartitarioBodyTemplatePath)
          ).result(binding)
        )
      ...
      def render_footer(opts={})
        ...
        footer.write(
          ERB.new(
            IO.read(Helpers::ScadenzarioHelper::PartitarioFooterTemplatePath)
          ).result(binding)
        )
...
#      def btn_stampa_click(evt)
#        Wx::BusyCursor.busy() do
#          template = Helpers::ScadenzarioHelper::PartitarioFornitoriTemplatePath
#          generate(template)
#        end
#      end
#
...
#        report.add_field :intestazione, ["Report Partitario Fatture Fornitori", (filtro.al.blank? ? '' : "al #{filtro.al.to_s(:italian_date)}")].join(' ')
#      end
#
#      def render_body(report, whatever=nil)
#        report.add_table("Report", self.result_set_lstrep_partitario, :header=>true) do |t|
...
#          t.add_column(:pagamento) {|row| Helpers::ApplicationHelper.currency(row[5])}
