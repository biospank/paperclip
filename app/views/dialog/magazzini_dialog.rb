...
module Views
  module Dialog
    class MagazziniDialog < Wx::Dialog
      ...
      def initialize(parent, attivi = true)
        ...
        xrc = Xrc.instance()
        xrc.resource.load_dialog_subclass(self, parent, "MAGAZZINI_DLG")
        ...
        xrc.find('lstrep_magazzini', self, :extends => ReportField)
        xrc.find('wxID_OK', self, :extends => OkStdButton)
        xrc.find('wxID_CANCEL', self, :extends => CancelStdButton)

        # lista magazzini

        lstrep_magazzini.column_info([{:caption => 'Nome', :width => 150},
                                      {:caption => 'Descrizione', :width => 150}])
        lstrep_magazzini.data_info([{:attr => :nome},
                                   {:attr => :descrizione}])
      ...
      def btn_ricerca_click(evt)
        ...
        self.result_set_lstrep_magazzini = ctrl.search_for_magazzini()
        if result_set_lstrep_magazzini.empty?
          Wx::message_box("Nessun record trovato.",
                  'Info',
                  Wx::OK | Wx::ICON_INFORMATION, self)
          txt_ricerca.activate()
        else
          lstrep_magazzini.display(result_set_lstrep_magazzini)
        end
      end

      def btn_ok_click(evt)
        if(lstrep_magazzini.get_selected_item_count() == 0)
          Wx::message_box("Seleziona un magazzino.",
                'Info',
                Wx::OK | Wx::ICON_INFORMATION, self)
