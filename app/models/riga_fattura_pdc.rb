...
module Models
  class RigaFatturaPdc < ActiveRecord::Base
    ...
    validates_presence_of :aliquota, 
      :if => Proc.new { |riga_pdc| riga_pdc.imponibile.nil? || riga_pdc.imponibile.zero? },
      :message => "Associare un codice aliquota oppure inserire l'imponibile."
    validates_presence_of :imponibile,
      :if => Proc.new { |riga_pdc| riga_pdc.aliquota.nil? },
      :message => "Inserire l'imponibile oppure associare un codice aliquota."
